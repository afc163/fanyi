...
function makeSureFileExisted(filePath) {
  // 若不存在，立即创建
  if (!fs.existsSync(filePath)) {
    const allPath = filePath.split('/');
    let nextPath = '';
    // 循环创建多级路径
    for (let i in allPath) {
      nextPath = nextPath + '/' + allPath[i];
      if (allPath[i].includes('.txt')) {
        fs.writeFileSync(filePath, '', { flag: 'a' });
      } else {
        !fs.existsSync(nextPath) && fs.mkdirSync(nextPath);
      }
    }
  }
}
...
exports.searchList = function (args) {
  ...
  let targetContent;
  // 与配置放在一起
  makeSureFileExisted(searchFilePath);

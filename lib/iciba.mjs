...
export function printIciba(word, message, options = {}) {
  ...
  log(`${gradientBg(` ${matchedKey} `, chalk)} ${chalk.gray('~')} ${chalk.yellow('iciba.com')}`);
  ...
  for (const item of matched.means || []) {
    ...
    log(`${chalk.gray('- ')}${part}${gradientGreen(meaning, chalk)}`);
  ...
  if (related.length) {
    ...
    for (const item of related) {
      log(`${chalk.gray('- ')}${chalk.redBright(item.key)}  ${chalk.gray(item.paraphrase)}`);
    }
  }

    ...
    if (item.ew.some(function(w) { return w === word })) {
      item.def.forEach(function(def) {
        def.dt.forEach(function(obj) {
          ...
          if (meaning) {
            log((' - ').faint + highlight(meaning.green, word, 'green'));
            i += 1;
          }
        });
      });
...
function highlight(string, key, defaultColor) {
  defaultColor = defaultColor || 'faint';
  string = string.replace(new RegExp('(.*)(' + key + ')(.*)', 'gi'), '$1$2' + '$3'[defaultColor]);
  return string.replace(new RegExp('(.*?)(' + key + ')', 'gi'), '$1'[defaultColor] + '$2'.yellow);
}

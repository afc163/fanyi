...
exports.chatgpt = async (data, word, options) => {
  if (options.color === false) {
    chalk = initChalkWithNoColor();
  }
  ...
  log(firstLine + chalk.gray('  ~  ChatGPT'));

  const { ChatGPTAPI } = await import('chatgpt');
  const api = new ChatGPTAPI({
    apiKey: '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',
  });
  api.sendMessage(`你能帮我翻译一下 "${word}" 这个词么？并给出一些例句`).then(res => {

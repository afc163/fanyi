...
exports.chatgpt = async (word, options) => {
  ...
  const opts = { apiKey: options.openaiApiKey };
  const proxy = process.env.https_proxy || process.env.http_proxy || process.env.all_proxy;
  if (proxy) {
    opts.fetch = (url, options) => {
      options.agent = new HttpsProxyAgent(proxy);
      return fetch(url, options);
    };
  }
  if (options.openaiApiHost && options.openaiApiHost !== 'false') {
    opts.apiBaseUrl = `https://${options.openaiApiHost}/v1`;
  }
  const api = new ChatGPTAPI(opts);
